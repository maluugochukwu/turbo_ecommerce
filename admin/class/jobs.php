...
class Jobs extends dbobject
{
    ...
    public function saveJob($data)
    {
        ...
        if($data['operation'] == "new")
        {
            ...
            $count   = $this->doInsert("job_posting",$data,array('operation','op','id','_files'));
            if($count > 0)
            {
                 
                return json_encode(array('response_code'=>0,'response_message'=>'Job Created Successfully '.$id));
            }else
            {
                return json_encode(array('response_code'=>47,'response_message'=>'Job Creation Failed'));
            }
        }else
        {
            $count   = $this->doUpdate("job_posting",$data,array('operation','op','id'),array('id'=>$data['id']));
            
            if($count > 0)
            {
                return json_encode(array('response_code'=>0,'response_message'=>'Job Updated Successfully'));
            }else
            {
    ...
    public function jobList($data)
    {
		...
		$columner = array(
			...
			array( 'db' => 'location',  'dt' => 2, 'formatter'=>function($d,$row){
                return $this->getitemlabel('lga','state_code',$d,'State');
            } ),
			array( 'db' => 'closing_date',  'dt' => 3),
            array( 'db' => 'company_name',  'dt' => 4,'formatter' => function($d,$row){
                return $this->getitemlabel('company','id',$d,'company_name');
            }),
            array( 'db' => 'description',  'dt' => 5),
            array( 'db' => 'id',  'dt' => 6,'formatter' => function($d,$row)
                {
                $id = $d;
                return '<a class="badge badge-warning" onclick="getModal(\'setup/job_setup.php?op=edit&id='.$id.'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Edit Job</a>  | <a class="badge badge-danger" onclick="deleteJob(\''.$id.'\')"  href="javascript:void(0)" >Delete Job</a>';
                }
            ),
