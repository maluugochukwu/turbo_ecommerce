<?php
class SocialMediaAddress extends dbobject
{
    private $response   = "";
    ...
    public function saveSocialMediaAddress($data)
    {
        ...
        if($data['operation'] == "new")
        {
            ...
            $count   = $this->doInsert("social_media_address",$data,array('operation','op','id','_files'));
            if($count > 0)
            {
                 
                return json_encode(array('response_code'=>0,'response_message'=>'social media address Created Successfully '.$id));
            }else
            {
                return json_encode(array('response_code'=>47,'response_message'=>'social media address Creation Failed'));
            }
        }else
        {
            $count   = $this->doUpdate("social_media_address",$data,array('operation','op','id'),array('id'=>$data['id']));
            
            if($count > 0)
            {
                return json_encode(array('response_code'=>0,'response_message'=>'social media address Updated Successfully'));
            }else
            {
    ...
    public function socialMediaAddressList($data)
    {
        $table_name    = "social_media_address";
		$primary_key   = "id";
		$columner = array(
			array( 'db' => 'id', 'dt' => 0 ),
			array( 'db' => 'name',  'dt' => 1 ),
