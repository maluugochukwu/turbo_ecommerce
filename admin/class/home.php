...
class Home extends dbobject
{
   public function homeList($data)
    {
		...
		$columner = array(
			...
			array( 'db' => 'value', 'dt' => 2, 'formatter' => function( $d,$row ) {
                ...
                if($type == "BANNER")
                {
                    return '<img class="img-thumbnail" src="'.$d.'" width="50" height="50" /><span class="badge badge-success" style="cursor:pointer" onclick="getModal(\'setup/home_image.php?op=edit&id='.$id.'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Edit Image</span>';
                }else
                {
                    return $d;
                }
            ...
            array( 'db' => 'id',     'dt' => 3, 'formatter' => function( $d,$row ) {
                ...
                {
                    return "";
                }elseif($type == "BANNER_TEXT")
                {
                    return '<a class="btn btn-primary" onclick="getModal(\'setup/banner_text.php?op=edit&id='.$d.'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Edit </a>';
                }
                elseif($type == "BANNER_SUBTITLE")
                {
                    return '<a class="btn btn-primary" onclick="getModal(\'setup/banner_text.php?op=edit&id='.$d.'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Edit </a>';
                }
                else
                {
                    return '<a class="btn btn-primary" onclick="getModal(\'setup/home_setup.php?op=edit&id='.$d.'\',\'modal_div\')"  href="javascript:void(0)" data-toggle="modal" data-target="#defaultModalPrimary">Edit </a>';
                }
...
//		$filter = " AND role_id='001'";
    ...
    public function saveHome($data)
    {
        ...
        if(!$validation['error'])
        {
            ...
            {
                    $count = $this->doInsert("home_page",$data,['op','operation','id']);
                    if($count > 0)
                    {
                        return json_encode(array('response_code'=>0,'response_message'=>'Home page Created Successfully'));
                    }else
                    {
                        return json_encode(array('response_code'=>996,'response_message'=>'Home page could not be created'));
                    }
            }else
            {
                $count = $this->doUpdate("home_page",$data,['op','operation','id'],["id"=>$data['id']]);
                if($count > 0)
                {
                    return json_encode(array('response_code'=>0,'response_message'=>'Home page updated Successfully'));
                }else
                {
                    return json_encode(array('response_code'=>996,'response_message'=>'Home page could not be updated'));
                }
            }
        }else
        {
...
//                    var_dump($ff);
    ...
    public function setHomeImage($data)
    {
        $file_data        = $data['_files'];
        $path        = 'uploads/';
        ...
        if($data['operation'] == "new")
        {
            $count = $this->doInsert("home_page",["type"=>"BANNER","created"=>date("Y-m-d h:i:s")],[]);
            if($count > 0)
            {
                $insert_id = mysqli_insert_id($this->myconn);
            }else
        ...
        {
            $full_path = $ff['data'];
            $sql       = "UPDATE home_page SET value = '$full_path' WHERE id = '$insert_id' LIMIT 1";
            $count     = $this->db_query($sql,false);
    ...
    public function setBannerText($data)
    {
        if($data['op'] == "new")
        {
            $this->doInsert("home_page",['type'=>'BANNER_TEXT','created'=>date('Y-m-d h:i:s'),'value'=>$data['banner_text']],[]);

            $this->doInsert("home_page",['type'=>'BANNER_SUBTITLE','created'=>date('Y-m-d h:i:s'),'value'=>$data['banner_subtitle']],[]);
            return json_encode(array('response_code'=>'0','response_message'=>'Successful'));
        }
        else
        {
            $this->doUpdate("home_page",['type'=>'BANNER_TEXT','created'=>date('Y-m-d h:i:s'),'value'=>$data['banner_text']],[],['type'=>'BANNER_TEXT']);

            $this->doUpdate("home_page",['type'=>'BANNER_SUBTITLE','created'=>date('Y-m-d h:i:s'),'value'=>$data['banner_subtitle']],[],['type'=>'BANNER_SUBTITLE']);
            return json_encode(array('response_code'=>'0','response_message'=>'Successful'));
        }
